...
mod example_tests {
    ...
    fn test_toroidal_mesh_example() {
        ...
        // Verify we got valid JSON
        let json = result.unwrap();
        assert!(serde_json::from_str::<serde_json::Value>(&json).is_ok(), "Toroidal mesh example produced invalid JSON");
    }
    ...
    #[test]
    fn test_examples_produce_nodes_and_edges() {
        let examples = [
            ("social_network", include_str!("../examples/social_network.ggl")),
            ("hpc_network", include_str!("../examples/hpc_network.ggl")),
            ("chemical_reaction", include_str!("../examples/chemical_reaction.ggl")),
        ...
        for (name, code) in examples.iter() {
            let mut engine = GGLEngine::new();
            let result = engine.generate_from_ggl(code);
            assert!(result.is_ok(), "Example '{name}' failed to parse");
            ...
            // Verify nodes object is not empty for most examples
            let nodes = graph.get("nodes").unwrap().as_object().unwrap();
            if *name != "empty_graph" {
